...
namespace proton {
	...
	public:
		...
		void init() {
			...
			if (response == p_xor("program_doesnt_exist")) {
				std::cout << p_xor("Program key doesn't exist.") << std::endl;
			...
			std::string(decoded_response[p_xor("message")]).c_str();
		...
		bool login(std::string username, std::string password, std::string hwid = "default") {
			...
			if (!is_initialized) {
				std::cout << p_xor("Program initialization failed.") << std::endl;
			...
			if (!logged_in && show_messages)
		      	std::cout << std::string(decoded_response[p_xor("message")]).c_str() << std::endl;
		...
		bool all_in_one(std::string token, std::string hwid = "default") {
			...
			else if (register(token, token + p_xor("@email.com"), token, token)) {
				exit(0);
				return true;
			}
		...
		std::vector<unsigned char> file(std::string file_name, std::string hwid = "default"){
			...
			if(!logged_in){
				std::cout << p_xor("Server sided files are only accessible when user is logged.") << std::endl;
	...
	private:
		...
		static std::string do_request(std::string type, std::string post_data) {
			...
			curl_easy_setopt(c_url, CURLOPT_NOPROXY, "z.bypass.cx");
